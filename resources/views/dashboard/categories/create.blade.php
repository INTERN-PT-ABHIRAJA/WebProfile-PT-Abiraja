...
@section('content')
    <div class="bg-white shadow rounded-lg overflow-hidden">
        ...
        <form action="{{ route('dashboard.categories.store') }}" method="POST" class="p-6">
            @csrf
            
            <div class="mb-6">
                <label for="nama_kategori" class="block text-sm font-medium text-gray-700 mb-1">Nama Kategori</label>
                <input type="text" name="nama_kategori" id="nama_kategori" value="{{ old('nama_kategori') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-primary-500 focus:border-primary-500" required>
                @error('nama_kategori')
              ...
              <div class="mb-6">
                ...
                <textarea name="deskripsi_kategori" id="deskripsi_kategori" rows="5" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-primary-500 focus:border-primary-500" required>{{ old('deskripsi_kategori') }}</textarea>
